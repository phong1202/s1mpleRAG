...
def _make_document(marker: str) -> Document:
    return Document(
        sha256_hash=marker, filename="x.pdf", object_key=f"raw/{marker}.pdf", size_bytes=1
    )


async def _hash_exists(sha256_hash: str) -> bool:
    ...
    try:
        async with engine.connect() as connection:
            result = await connection.execute(
                text("SELECT 1 FROM documents WHERE sha256_hash = :sha256_hash"),
...
async def _delete_hash(sha256_hash: str) -> None:
    engine = create_async_engine(TEST_DATABASE_URL)
    try:
        async with engine.begin() as connection:
            await connection.execute(
                text("DELETE FROM documents WHERE sha256_hash = :sha256_hash"),
                {"sha256_hash": sha256_hash},
            )
    finally:
        await engine.dispose()
...
async def test_successful_operation_commits_and_is_visible_from_another_connection():
    ...
    try:
        async with session_scope() as session:
            session.add(_make_document(marker))
            await session.flush()

        assert await _hash_exists(marker) is True
    finally:
        await _delete_hash(marker)
...
async def test_app_exception_after_a_flush_is_rolled_back():
    ...
    try:
        ...
        assert await _hash_exists(marker) is False
    finally:
        await _delete_hash(marker)
